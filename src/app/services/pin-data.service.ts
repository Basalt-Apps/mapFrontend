...
export class PinDataService {
  private pins$ = new BehaviorSubject<ReadonlyMap<number, PinModel>>(new Map());
  ...
  public getAll(): Observable<PinModel[]> {
    return combineLatest([this.ids$, this.pins$]).pipe(
      map(
        ([ids, pins]: [number[], ReadonlyMap<number, PinModel>]): PinModel[] =>
          ids
            .map((id: number): PinModel | undefined => pins.get(id))
  ...
  private load(): void {
    this.pinService.getAll().subscribe((pins: PinModel[]) => {
      const ids = [];
  ...
  private update(ids: number[], pinMap: Map<number, PinModel>): void {
    this.ids$.next(ids);
