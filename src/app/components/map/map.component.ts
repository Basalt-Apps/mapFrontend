...
export class MapComponent implements OnInit {
  ...
  public clicked$ = new BehaviorSubject<PinCreateComponentMetadata | null>(
    null,
  );
  ...
  public ngOnInit(): void {
    ...
    this.admin$ = this.map$.pipe(
      filter(Boolean),
      combineLatestWith(
        this.authService.getTokenPayload().pipe(filter(Boolean)),
      ),
  ...
  public onMapInit(): void {
    const mapImg = this.elRef.nativeElement.querySelector(
      '#mapImg',
    ) as HTMLImageElement;
  ...
  private draggingLogic(mapImg: HTMLImageElement): void {
    ...
    mapImg.addEventListener('dragstart', (event: MouseEvent) =>
      event.preventDefault(),
    );
    mapImg.addEventListener('contextmenu', (event: Event) =>
      event.preventDefault(),
    );
  ...
  private scrollingLogic(mapImg: HTMLImageElement): void {
    mapImg.addEventListener('wheel', (event: WheelEvent) => {
      ...
      this.mapPos = this.mapPos.add(
        oldSize.subtract(newSize).hadamardProduct(mapSpacePos),
      );
  ...
  private handleClick(mapImg: HTMLImageElement, event: MouseEvent): void {
    this.admin$
      .pipe(
        ...
        switchMap(
          (set: boolean): Observable<true | null | void> =>
      ...
      .subscribe(() => {
        ...
        this.clicked$.next({
          ...
          positionTransformed: client
            ...
            .min(
              new V2(
                (window.visualViewport?.width ?? 1920) - 300,
                (window.visualViewport?.height ?? 927) - 50,
              ),
            ),
        });
      });
  ...
  private placePins(mapImg: HTMLImageElement): void {
    ...
    this.pins$ = this.map$.pipe(
      ...
      filter(
        ([id, pins]: [number, PinModel[]]): boolean =>
          id >= 0 && pins.length > 0,
      ),
      map(([id, pins]: [number, PinModel[]]): OmitPinPos[] =>
        pins
    ...
    this.pinPositions$ = this.map$.pipe(
      ...
      filter(
        ([id, pins]: [number, PinModel[], V2, number]): boolean =>
          id >= 0 && pins.length > 0,
      ),
      map(
        ([id, pins, mapPos, zoomLevel]: [
          number,
          PinModel[],
          V2,
          number,
          ...
          pins
            .filter((pin: PinModel) => pin.MapID === id)
            .map(
              (pin: PinModel): Observable<V2> =>
                of(this.transformPos(pin.Pos, mapImg, zoomLevel, mapPos)),
            ),
      ),
    );
    ...
    return pos
      ...
      .subtract(
        new V2(1, 1)
          .multiply(
            (this.basePinSize *
              (zoomLevel / this.zoomPinMod + 100 * (1 - 1 / this.zoomPinMod))) /
              100,
          )
