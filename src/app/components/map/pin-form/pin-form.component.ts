...
@Component({
  ...
  standalone: true,
  imports: [PopupComponent, AsyncPipe, ReactiveFormsModule],
...
export class PinFormComponent implements OnInit {
  ...
  public ngOnInit(): void {
    this.form = new FormGroup<ControlsOf<PinCreateForm>>({
      ...
      pos: new FormControl<V2 | null>(null, {
        validators: Validators.required,
      }),
    });
  }
  ...
  public onSubmit(): void {
    this.clicked$
      .pipe(
        ...
        filter(
          (
            data: [PinCreateComponentMetadata | null, MapModel | null],
          ): data is [PinCreateComponentMetadata, MapModel] =>
            !!data[0] && !!data[1],
        ),
        map(
          ([data, map]: [PinCreateComponentMetadata, MapModel]): [
            V2,
            number,
          ] => [data.position, map.ID],
        ),
